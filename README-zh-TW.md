...
AI 幫你的 Threads / IG / X / LinkedIn 貼文打分、抓問題、改到能發。貼上草稿，拿到 0-100 分數 + 逐句診斷 + 改寫版——全部根據你的個人品牌校準。
...
把 `SKILL-zh-TW.md`（或 `SKILL.md`）的內容複製到你的 system prompt 或自訂指令。評分邏輯適用於任何能遵循結構化指令的 LLM。
...
評分基於以下公開研究：
- 馬斯克本人公開發言（已驗證推文原文，[連結](https://x.com/elonmusk/status/1875355425601999255)）
- X 演算法開源碼（停留時間權重、原生內容加分、原創性信號）
- Meta 官方內容分發指南（Engagement Bait 政策）
- Buffer 分析 250 萬篇 Threads 貼文（最佳發布時間）和 4500 萬篇貼文（最佳內容格式）

完整來源列表附連結在 `SKILL-zh-TW.md` 底部。
